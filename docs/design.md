...
| BOLL(20) | 40 | CCI(20) | 43 |
| KDJ(9,3,3) | 43 | RSI(14) | 7 |
| **九个指标一整套** | **215** | | |

百万步的回测在指标上一共花 0.2 秒。为省这点开销去换一个会随步数漂移的增量
实现，不划算。

> **2026-09-02 修正**：窗口的统计量原先直接遍历 `w.buf`，那是【物理顺序】——
> 环形缓冲的旋转位置取决于已经推入了多少根，于是同一份数据从不同起点喂进来，
> 累加顺序就不同，浮点结果差最后一位。窗口类指标在数学上只依赖窗口里那 n 根，
> 本该与起点无关。改成按时间顺序走两段连续区间后表里的数字涨了约 10%
> （MA(20) 11→13、BOLL 35→40、一整套 202→215），这是为逐位可复现付的价。
>
> 一开始写成带取模的单循环，MA(20) 直接从 11ns 涨到 35ns——三倍。取模是每元素
> 一次的代价，而拆成 `buf[i:]` + `buf[:i]` 两段既保住顺序，又让编译器照常向量化。
>
> 照出它的过程值得记：起因是 okx-decision-strategy 报了一组它自己实测的收敛点，
> 里面测的是 `kdj.j` 而我的测试只验 `.k`。补齐字段后，KDJ 的 **TV 口径**
> 在一份数据上 13 根收敛、在另一份要 14 根——而它是纯窗口指标，
> 不该有这种差别。差的那 1 ULP 不会报错，只会在恰好相等的比较上翻面。
