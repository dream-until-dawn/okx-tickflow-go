...
草稿阶段我把默认定成了 TV，理由是「国际主流」。**那是个错的默认**——这是给 OKX
用的库，而 OKX 自己的行情界面用的是 CN 口径。

2026-09-01 拿 ETH-USDT-SWAP 的日线在 OKX 平台上逐行比对最近 10 根的 KDJ(9,3,3)
与 MACD(12,26,9)：CN 对得上，TV 对不上。v1.1.0 起默认改为 CN。

判别之所以干脆，正是因为两处**永久**差异：MACD 的柱 CN 恰是 TV 的两倍（当时是
23.94 vs 11.97），KDJ 的 K 差得更明显（63.99 vs 54.93）。而 DIF/DEA 在 500 根之后
两套已完全相同——播种差异是暂时的，反倒不能用来判别。

那十行值固化在 `indicator/okx_convention_test.go` 里，配一条反向测试
（TV 必须**对不上**）。只有正向那条的话，它只说明「CN 算出来是这些数」，
说明不了「为什么默认要选 CN」。
...
### MarkPx / IdxPx：默认留空，但拿得到

标记价和指数价在 OKX 是**各自独立的 K 线序列**（`mark-price-candles` /
`index-candles`），不是普通行情里的字段。

`ToBar` 默认不填它们，要显式给：

```go
simbar.ToBar(inst, v.Candle(), simbar.WithMarkPx(v.MarkPx()))
```

不给时 simulator 的 `Bar.markPx()` 会用 `Last` 顶替——**那是个会静默产生假阴性的
退化**，见第八节。`IdxPx` 缺失时按 index 触发的算法委托会被跳过并在 `StepResult`
里说明原因，它不拿别的价格顶替，那个行为是对的。

> v1.1 之前本库根本拿不到这两条序列（上游没有端点），只能留空。
> v1.2.0 补齐了，见第八节——那一节也记着「把外部限制当成可接受的现状」
> 这个判断错在哪。
...
v1.1 之前，`adapter/simbar` 不填 `Bar.MarkPx`，记账内核就退回用最新成交价顶替。
当时的理由是「上游 okx-api-v5-go 没有 mark-price-candles 端点」——**那是个事实，
但把它当成可以接受的现状是错的**。

okx-grid-strategy（网格引擎）那边把这件事的代价说清楚了：OKX 的强平按标记价判定，
用成交价意味着**影线会制造出真实不会发生的强平**。对尾部风险就是强平的策略
（做多网格正是），这会让本来可用的参数组合在扫描里被淘汰——而且是**假阴性，
扫描结果里不留任何痕迹**。

上游 v0.2.0 补齐了 `MarkPriceCandles` / `IndexCandles` 及其 history 变体，
v1.2.0 接上。

### 命名空间：为什么不能用合成 instId

`Store` 按 `(instId, bar)` 索引，而标记价的 `BTC-USDT-SWAP/1m` 与成交价的
`BTC-USDT-SWAP/1m` 是**两条不同的序列，同一个键**。

拼一个 `BTC-USDT-SWAP:mark` 之类的合成 instId 能糊过去，但那个合成键会渗进
`Meta`、`Coverage`、`SeriesID`，以后再拆就是破坏性变更。正经做法是用命名空间：

```
<root>/candles/       成交价
<root>/mark-candles/  标记价
<root>/index-candles/ 指数价
```

`candles/` 那一层本来就是为此留的（见「布局」），标记价是它的第一个用户。

**命名空间绑在 Store 构造上**（`segfile.Open(root, segfile.Mark)`），而不是给
`Store` 接口加一个 kind 参数——后者是 v1 接口的破坏性变更，要换模块路径。
绑在构造上则 `tickflow.Store` 一个字节都不用改，`Syncer` 与 `Feed` 拿到哪个
Store 就读哪个。

**写锁跟着挪进命名空间**（`<root>/<kind>/.lock`）。不然同一进程里开一个成交价
Store 加一个标记价 Store 就会自己把自己锁住，而它们写的本就是两堆互不相干的文件。
v1.1 及更早的写者持有根上的旧锁却写 `candles/` 下的文件，所以旧锁还在时，
成交价这个命名空间要当作已被占用——那是个很窄但真实的双写窗口。

### 对齐规则：同 ts，不是「最后一根已收盘」

辅周期用的是「最后一根已收盘」，标记价用的是**同一个 ts**。

差别在于二者与主周期的关系不同：辅周期这一根**还没走完**，用它就是偷看未来；
而标记价 K 线与主周期这一根是**同时收盘**的，它的收盘价在这一刻就已知。

主周期有空洞时，标记价**不会错位**到别的时刻上去。这一条是测试照出来的：
环形缓冲会复用槽位，最初的实现没在 push 时清空标记价那一格，于是标记价缺根时
读到的是 capN 根之前那一根的值——**比没有标记价更糟，它看起来是对的**。

### 实测

- 200 根 BTC-USDT-SWAP 1H：平均振幅 成交价 **0.6433%** vs 标记价 **0.6300%**，
  标记价更窄的占 146/200。这就是「标记价由指数价平滑而来」的实证，
  也是为什么用成交价会多算出强平。
- 500 根 ETH-USDT-SWAP 日线走完整条链，记账内核用**默认配置**全程无错，
  其中 **474/500 步的标记价与最新价不同**——证明进来的确实是另一条序列。
- 反向测试拆成四个用例：默认配置下不给标记价必须报错、NaN 等同于不给、
  给了就该过、显式打开 `AllowMarkPxFallback` 时不给也能跑且用最新价顶替。

> 反向那组起初只有一个用例，且**显式**配了开关。记账内核 v1.0.0 把这个开关反了
> 过来（`RequireMarkPx` → `AllowMarkPxFallback`，默认从「允许退回」变成「必须给」），
> 而显式配了开关的测试**照不出默认值被翻转**。改成用默认配置跑正向、并把反向拆成
> 「默认必须报错」与「显式开退回则能跑」两组，这件事才测得住。
>
> 记账内核把默认翻过来是对的，但**理由要说准**。它最初给的依据是「标记价与普通
> K 线同深，降级完全可避免」——那条后来被它自己撤回了：那次测量是在【模拟盘】上
> 做的，模拟盘的两条序列在同一天一起截断，于是「一起返回空」被读成了「一样深」。
>
> 本库随后在生产环境自己复测：标记价有一条硬线，而且**分两档**——日线档港时
> `2020-01-01`，日内档（`1H`/`15m`/`1m`）`2020-01-03`。成交价的边界同样随周期变。
> 完整的表在 [contract.md](contract.md)。
>
> 所以正确的说法是「**从对应周期的硬线起**可避免」。早于它的区间标记价根本不存在，
> 那时打开 `AllowMarkPxFallback` 是正当的，不是将就。
>
> 这条结论**改过两次，两次是同一类错**：
>
> 1. 第一次写成「两条同深」——来源方在模拟盘上测的，两条一起截断被读成一样深；
> 2. 第二次内容对了，却**只量了 1D 而表里没有 bar 列**，被两个下游仓照抄，
>    其中一个据此写了「还有两天可拉」——那两天在 1m 上根本不存在。
>
> 形态是同一个：**「两条都取不到」与「两条一样深」长得一样；「1D 的边界」与
> 「与周期无关的边界」在表格上也长得一样。** 一个不带前提的结论，和一个错的结论
> 一样能传染——所以给结论标来源与重跑方式还不够，**前提也得写进表里**。
...
- **前复权 / 后复权** —— 加密市场没有这个概念。
- **资金费率** —— 见上。
- **指标注册表与字符串规格**（`"ema:20"` 这类配置驱动的写法）—— 构造函数已经够用。
- **多进程并发写** —— 一个数据目录同一时刻只允许一个写者，由 `Open` 的写锁保证，
  第二个写者会拿到 `ErrLocked`。并发**读**是支持的（`OpenReadOnly`）。

---

## 十一、实测记录

实测出来的数都记在这里。每条的重跑方式见 [contract.md](contract.md) 的「置信度」一节。

### 2026-08-31：周期对齐与历史深度（BTC-USDT-SWAP）
...
### 2026-09-01：OKX 平台的指标口径

本库最后一处标着「未实测」的地方。拿 ETH-USDT-SWAP 的日线（港时对齐）在 OKX
平台上比对最近 10 根：

| | TV 口径 | CN 口径 | OKX 平台 |
|---|---:|---:|---|
| MACD 柱（港时 08-31） | 11.97 | **23.94** | 对上 CN |
| KDJ 的 K（港时 08-31） | 54.93 | **63.99** | 对上 CN |

MACD 的 DIF/DEA 两套已收敛到完全相同（155.42 / 143.45），所以判别只能看柱与 KDJ。

结论：**默认口径改为 CN**，并把那十行固化成回归测试。
...
（`TestLiveAlignment`），但交易所改起来不必通知谁。真出问题时
`SyncReport.Misaligned` 会不为零，用 `RegisterFixedPeriod` 覆盖即可。
