...
- 200 根 BTC-USDT-SWAP 1H：平均振幅 成交价 **0.6433%** vs 标记价 **0.6300%**，
  ...
  也是为什么用成交价会多算出强平。
- 500 根 ETH-USDT-SWAP 日线走完整条链，记账内核开着 `RequireMarkPx`
  全程无错，其中 **474/500 步的标记价与最新价不同**——证明进来的确实是另一条序列。
- 反向测试：不给标记价、或给 NaN，开着 `RequireMarkPx` 的记账内核必须**报错**。
  没有这一条，正向测试只说明「给了能跑通」，说明不了那个开关有用。
