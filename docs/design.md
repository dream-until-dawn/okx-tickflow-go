...
>
> 本库随后在生产环境自己复测（`TestLiveMarkPriceHistoryFloor`，5 个合约）：
> 标记价有一条硬线，**港时 2020-01-01**。之后上线的合约同深；之前上线的一律被截到
> 那天——BTC-USD-SWAP 差了 379 天。
>
> 所以正确的说法是「**港时 2020-01-01 起**可避免」。早于它的区间标记价根本不存在，
> 那时打开 `AllowMarkPxFallback` 是正当的，不是将就。
>
> 这件事本身值得记：**「两条都取不到」和「两条一样深」在数据上长得一模一样**。
> 这正是 contract.md 要给每条结论标来源与重跑方式的理由——署了名的二手结论，
> 也可能是从一个测不到边界的环境里得出来的。
