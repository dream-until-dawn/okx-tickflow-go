...
| 日内档的边界是固定还是滚动窗口 | **未判定**。单次测量看不出；但 BTC 与 ETH 的日内档日期不同，说明它至少不是全局统一的滚动窗口 | 若是滚动的，回测区间会随时间悄悄缩短——`SyncReport.Gaps` 会把取不到的段列出来 |
...
> 这些都是**观察结论，不是官方承诺**。尤其「日内档是不是个滚动窗口」本库
> **没有判定**——单次测量看不出固定还是滚动，而 BTC 与 ETH 的日内档日期不同，
> 说明它至少不是一个全局统一的滚动窗口。边界一变，
> `TestLiveHistoryFloorVariesByBar` 会先报。
